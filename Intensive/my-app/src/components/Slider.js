...
class Slider extends React.Component {
	constructor(props){
		super(props);
		this.state = {			//Объект state в котором хранятся все свойства слайдера
			slides: [			//Массив со слайдами а так же с свойствами 
				{
		...
		this.state.max = this.state.slides.length;	//Назначение свойству max кол-во слайдов
	...
	dots(index,event){			//Переключение по точки
		this.setState({
			active: index
		})
	}
	...
	setSliderStyles(){			//Переключение слайдера
		const transition = this.state.active * - 100 / this.state.slides.length //Длинна переключения слайдера
